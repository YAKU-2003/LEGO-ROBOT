"""Drive the LEGO Double Motor up to a stationary AprilTag, using an iPhone
camera mounted ON the robot.

The tag is a plain, fixed target -- nothing spins it. The car's job is simply:

    aim    centre the tag in the picture, then
    drive  in until the tag's TOP EDGE nearly touches the top of the yellow box
...
# Rotate the incoming frame 90 deg, e.g. to test a vertical (portrait) mount
# in software before physically remounting the phone. cv2.ROTATE_90_CLOCKWISE,
# cv2.ROTATE_90_COUNTERCLOCKWISE, cv2.ROTATE_180, or None for no rotation.
ROTATE_90 = cv2.ROTATE_90_CLOCKWISE
CAMERA_HFOV_DEG = 65  # the PHYSICAL camera's horizontal FOV; if ROTATE_90 is set,
# this is measured along what is now the frame's SHORT (vertical) edge -- re-measure
# it for accurate distance readings, the bearing/aim math still works approximately either way.
...
MAX_PULSE_SECONDS = 0.4  # was 0.8 -- even the 0.08s floor was losing the tag, so
# cap the top end lower too: this hardware moves a lot more per second than the
# modeled rate expects (BLE command latency + motor lurch), so a shorter cap on
# duration is the other half of slowing this down, alongside the lower speeds below.
...
SETTLE_SECONDS = 1.5  # was 1.0 -- give the BLE command / motor coast more room to
# finish before measuring, since pulses have been landing further than intended.
SAMPLE_FRAMES = 5  # readings (median) used for each decision

# Aiming the car. Error is degrees the car must turn right.
TURN_STOP_DEG = 2.0
TURN_START_DEG = 5.0
TURN_SPEED = 8  # percent -- was 15, slowed down: a full 15% pulse was overshooting badly
INITIAL_TURN_RATE = 30.0  # deg/s guess at TURN_SPEED; re-measured as it goes
# Hard cap on how far a single turn pulse is allowed to swing the tag, even if
# the learned rate is under-estimated (stream lag makes early rate readings
# unreliable). CAMERA_HFOV_DEG/2 is where the tag falls off the edge of frame
# entirely, so this stays well inside that with margin to spare.
MAX_TURN_DEGREES_PER_PULSE = 8.0  # was 15.0, tightened alongside the slower TURN_SPEED

# Driving the car. Error is pixels of growing room still left in the frame.
RANGE_STOP_PX = 8.0
RANGE_START_PX = 18.0
DRIVE_SPEED = 15  # percent -- was 30, slowed down to match the gentler turn pulses
...
def tag_yaw_deg(corners, focal_px):
    ...
    Display only now -- nothing corrects it, since the tag doesn't move on
    its own -- but it's still useful for judging how you've mounted the tag.
    ...
    try:
        ...
        while True:
            ...
            if target is None:
                ...
                if mode == "calibrate":
                    wheels, status = sign_finder.step(target, now)
            ...
            cv2.putText(frame, f"L {drive.last[0]}%  R {drive.last[1]}%   "
                               f"signs d{SIGNS['drive']:+d} t{SIGNS['turn']:+d}",
                        (10, 88), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 200, 0), 1)
            cv2.imshow("AprilTag Approach (iPhone)", frame)
